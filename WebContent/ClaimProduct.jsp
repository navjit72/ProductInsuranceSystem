...
<body>
	<ul class="nav nav-tabs justify-content-center">
		...
		<li class="nav-item"><a class="nav-link"
			href="RegisterProduct.jsp">Register Product</a></li>
		<li class="nav-item"><a class="nav-link active"
			href="ClaimProduct.jsp">Claim Product</a></li>
       ...
       SELECT pname from product where pId='${row.pId}';
				...
				<c:forEach var="rowx" items="${productIds.rows }">
					<tr>
						...
						<td><textarea rows="2" cols="25" class="form-control"
								name="issue"></textarea></td>
						<td><input type="radio" name="radiogroup"
							value="${row.pId},${row.serialNo },${row.purchaseDate }" /></td>
					</tr>
				</c:forEach>
			</c:forEach>
		</table>
		<input type="submit" value="Claim Product" class="btn btn-info"
			name="claim" />
