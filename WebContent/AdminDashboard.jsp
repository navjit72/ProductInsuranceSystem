				...
				<c:forEach var="rowx" items="${productIds.rows }">
					<tr>
						...
						<td><input type="radio" name="radiogroup"
							value="${row.username },${row.pId},${row.serialNo },${row.pDate },${row.claimDate },${row.status },${row.claimId}" /></td>
					</tr>
				</c:forEach>
			</c:forEach>

		</table>
		<input type="submit" value="Display Info" class="btn btn-info"
			name="display" />
	</form>
		...
		<c:choose>
			...
			<c:when test="${param.submitted and not empty param.radiogroup}">
				...
				<table class="table-bordered table-striped"
					...
					<tr>
						<td><input type="text" value="${data[2]}" disabled="disabled"
							class="form-control" /></td>
						<td><input type="text" value="${data[4]}" disabled="disabled"
							class="form-control" /></td>
						<td><input type="hidden" value="${data[6]}" name="id" /><select
				...
				<input type="submit" value="Update Claim Info" class="btn btn-info"
					name="update" />
			</c:when>
		</c:choose>
