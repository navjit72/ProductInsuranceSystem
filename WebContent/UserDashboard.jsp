...
<body>
	<ul class="nav nav-tabs justify-content-center">
		...
		<li class="nav-item"><a class="nav-link" href="ClaimProduct.jsp">Claim
				Product</a></li>
	...
	<form action="" method="post">
       ...
       SELECT pname from product where pId='${row.pId}';
		...
		<input type="submit" value="Display Product Info" class="btn btn-info"
			name="display" />
       ...
       SELECT * from claims where username='${cookie.currentUser.value}' and pId='${data[0]}' and serialNo='${data[1]}' and pDate='${data[2]}' ;
				...
				<c:choose>
					<c:when test="${claimDetails.rowCount==0 }">
						<div class="alert alert-danger" role="alert">
							<c:out value="No Claim records found!" />
						</div>
					</c:when>
					<c:otherwise>
