...
<body>
	...
	<fmt:formatDate type="date" pattern="yyyy-MM-dd" value="${ now }"
		var="currentDate" />
	<fmt:parseNumber value="${ now.time / (1000*60*60*24*365) }"
		integerOnly="true" var="currentTime" />
	...
	<fmt:parseNumber value="${ parsedDate.time / (1000*60*60*24*365) }"
		integerOnly="true" var="purchasedTime" />

	<c:if test="${not empty param.radiogroup}">
		<c:choose>

			<c:when
				test="${claimDetails.rowCount lt '3' and (currentTime-purchasedTime) lt '5'}">
			...
			<c:otherwise>
				<c:redirect url="ClaimProduct.jsp">
					<c:param name="errMsg"
						value="Product either passed 5 years warranty period or has been already claimed maximum number of times." />
				</c:redirect>
			</c:otherwise>
		</c:choose>
	</c:if>
